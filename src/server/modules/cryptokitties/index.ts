...
import { calculateCurrentPrice, getCryptoKittiesContractHistory } from './indexer';
import createModels, { CryptoKittiesModels } from './models';
...
const logEvent = async (
    ...
    web3: { eth: { getTransaction: (hash: string) => Promise<{ value: bigint }> } },
    emitter: { emit: (name: string) => void }
) => {
    ...
    if (event.event === 'Birth') {
        const tokenId = event.returnValues
            ? Number((event.returnValues as Record<string, unknown>).kittyId)
            : Number(event.tokenId);
    ...
    if (event.event === 'Pregnant') {
        ...
        const matronPartnerIds = (matron.partnerIds as number[] | undefined) ?? [];
        ...
        if (!sirePartnerIds.includes(matron.tokenId as number)) {
            ...
            sire.partnerIds = sire.partnerIds ? [...sire.partnerIds as number[], matron.tokenId as number] : [matron.tokenId as number];
            matron.partners = matron.partners ? (matron.partners as number) + 1 : 1;
            matron.partnerIds = matron.partnerIds ? [...matron.partnerIds as number[], sire.tokenId as number] : [sire.tokenId as number];
        }
        sire.cooldownEndBlock = (cooldowns[sire.cooldownIndex as number] / blocksPerSecond) + (blockNumber as number);
    ...
    try {
        ...
        if (startingPrice) {
            ...
            await Models.NFT.findOneAndUpdate(
                { tokenId },
                { sale, sire: !sale, currentPrice, startingPrice, endingPrice, duration, auctionStart: Number(auctionStart), auctionEnd: Number(auctionEnd) },
                { upsert: false }
            ).exec();
        } else {
            await handleTransferOrAuctionEnd(event, Models, web3, from, to, tokenId, emitter);
...
const handleTransferOrAuctionEnd = async (
    ...
    if (isSaleEnd || isSireEnd) {
        ...
        await Models.NFT.findOneAndUpdate({ tokenId }, { owner: to, $addToSet: { owners: to } }, { upsert: false }).exec();
...
const handleAuctionEnd = async (
    ...
    try {
        ...
        const unset: Record<string, unknown> = {
            sale: false,
            sire: false,
            $unset: { currentPrice: '', startingPrice: '', endingPrice: '', duration: '', auctionStart: '', auctionEnd: '' },
        };
...
const handleBirthEvent = async (event: Record<string, unknown>, Models: CryptoKittiesModels) => {
    ...
    if (event.matronId === 0 && event.sireId === 0) {
        ...
        birth.gen = (matron.gen as number) >= (sire.gen as number) ? (matron.gen as number) + 1 : (sire.gen as number) + 1;
    }
...
const runModule = (config: ModuleConfig) => {
    ...
    getCryptoKittiesContractHistory(
        name || 'cryptokitties',
        {
            ...
            deployed,
            increment,
            logEvent: (event) => logEvent(event, Models, web3, emitter),
        },
        eventsToWatch,
        web3
    ).catch((error) => {
