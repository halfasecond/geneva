...
const modules: ModuleFunction = (app, io, web3, db) => {
    ...
    // } else {
    //     console.log(`cryptokitties web3: ${cryptokittiesSocketUrl.replace(/\/[^/]+$/, '/…')}`)
    //     const cryptokittiesWeb3Connection = createWeb3Connection(cryptokittiesSocketUrl)
    //     cryptokittiesModule({
    //         app,
    //         io,
    //         web3: cryptokittiesWeb3Connection,
    //         db,
    //         name: 'cryptokitties',
    //         prefix: 'ck',
    //         deployed: 4605346,
    ...
    //         emitter,
    //     })
    //     console.log(`cryptokitties indexer: ${VITE_ENABLE_INDEXER === 'true' ? 'enabled' : 'subscription only'}`)
    // }
    ...
    const eliteSocketUrl = WEB3_SOCKET_URL_ELITE || process.env.WEB3_SOCKET_URL || ''
    if (!eliteSocketUrl) {
        console.warn('elite module skipped — set WEB3_SOCKET_URL_ELITE or WEB3_SOCKET_URL')
    } else {
        const eliteWeb3Connection = createWeb3Connection(eliteSocketUrl)
        eliteModule({
            app,
            io,
            web3: eliteWeb3Connection,
            db,
            name: 'vech',
            prefix: 'vech',
            deployed: 14035510,
        ...
        console.log(`elite (vech) indexer: ${VITE_ENABLE_INDEXER === 'true' ? 'enabled' : 'subscription only'}`)
    }
