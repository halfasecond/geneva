...
const routes = (Models: KittyInternationalModels): Router => {
    ...
    router.post('/', async (req: Request, res: Response) => {
        ...
        try {
            ...
            res.status(200).json({ token, address: recoveredSigner.toLowerCase() });
        } catch (error) {
            console.error('[kittyinternational-auth] login', error);
    ...
    router.post('/logout', async (req: Request, res: Response) => {
        ...
        try {
            ...
            const error = err as Error;
            console.error('[kittyinternational-auth] logout', error);
            res.status(500).json({ error: error.message });
        }
    });
